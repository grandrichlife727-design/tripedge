...
// ════════════════════════════════════════════
const FEATURES = [
  { id: "save", label: "Social Save", icon: "📌", desc: "Capture inspiration from the open web and turn it into a usable trip." },
  { id: "forecast", label: "Price Forecast", icon: "📈", desc: "Show whether the traveler should book now or wait for a better entry." },
  { id: "hacks", label: "Hack Finder", icon: "✂️", desc: "Surface booking tricks with the savings and risk spelled out clearly." },
  { id: "expenses", label: "Expenses", icon: "💰", desc: "Keep the shared-trip money layer clean, visible, and easy to settle." },
];

export default function NewFeaturesDemo({ initialFeature = "save", embedded = false }) {
        ...
        {embedded && (
          <div style={{ marginBottom: 22 }}>
            <div style={{
              ...
              <div>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#2A9D8F", textTransform: "uppercase", letterSpacing: 1.2, marginBottom: 4 }}>Tier 1 features</p>
                <p style={{ fontSize: 14, color: "#5C5142", maxWidth: 560 }}>{activeFeature.desc}</p>
              </div>
              <div style={{ background: "#E6F2FA", color: "#1A6DAD", border: "1px solid #C0DDF2", borderRadius: 999, padding: "6px 12px", fontSize: 11, fontWeight: 700, letterSpacing: 0.5 }}>PRO / PREMIUM</div>
            </div>
            <div style={{ display: "flex", gap: 8, background: "#EEE7DA", borderRadius: 16, padding: 6, width: "fit-content", marginBottom: 8, flexWrap: "wrap" }}>
              {FEATURES.map(f => (
                <button key={f.id} onClick={() => setActive(f.id)} style={{
                  background: active === f.id ? "#fff" : "transparent",
                  color: active === f.id ? "#2C2418" : "#8C7E6A",
                  border: active === f.id ? "1px solid #EDE8DD" : "1px solid transparent",
                  padding: "10px 16px", borderRadius: 12,
                  fontSize: 13, fontWeight: active === f.id ? 600 : 500,
                  fontFamily: "inherit", cursor: "pointer",
                  boxShadow: active === f.id ? "0 4px 14px rgba(44,36,24,0.08)" : "none",
