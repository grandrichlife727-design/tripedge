      ...
      {phase === "select" && (
        <div style={{ maxWidth: 680, margin: "0 auto", padding: "60px 32px" }}>
          ...
          <div className="fade-up" style={{
            ...
            {[
              { label: "Input", value: "Photos + trip stats" },
              { label: "Themes", value: "3 replay styles" },
              { label: "Output", value: "Share-ready story" },
            ].map((item, index) => (
              <div key={index} style={{ background: "#fff", border: "1px solid #EDE8DD", borderRadius: 14, padding: "14px 16px", textAlign: "left" }}>
                <p style={{ fontSize: 11, fontWeight: 700, color: "#A89E8C", textTransform: "uppercase", letterSpacing: 1.1, marginBottom: 5 }}>{item.label}</p>
                <p style={{ fontSize: 14, fontWeight: 600, color: "#2C2418" }}>{item.value}</p>
              </div>
            ))}
          </div>
          ...
          <p className="fade-up" style={{ animationDelay: "0.42s", marginTop: 12, textAlign: "center", fontSize: 12, color: "#8C7E6A" }}>
            Every replay keeps TripEdge branding and is designed to export cleanly for sharing.
          </p>
        </div>
      )}
